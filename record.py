...
def testUSB():
    ...
    try:
        if x==0:
            writeToScreen('Status', 'USB connected', ''),testConnection()
        elif x != 0: raise StandardError
    except StandardError : writeToScreen("Error:", 'USB disconnected', 'Please insert USB drive'), time.sleep(1)

def testConnection():
    try: 
        if result !=0 : record() 
        elif result == 0 : raise IOError
    except IOError : writeToScreen('Error:','No video input detected','Please connect HDMI or component source','') ,time.sleep(1)
